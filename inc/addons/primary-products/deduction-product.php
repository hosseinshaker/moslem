<?php
$title_page = "مدیریت موجودی محصولات اولیه";
...
if (isset($_POST['reduce_stock'])) {
    ...
    $reduceAmount = $_POST['reduce_amount'];
    ...
    if ($reduceAmount > 0) {
        ...
        if ($result_check_stock && $result_check_stock['productCount'] >= $reduceAmount) {
            // به‌روزرسانی موجودی در پایگاه داده
            $sql_update_stock = "UPDATE primaryproduct SET productCount = productCount - ? WHERE id = ?";
            $stmt_update_stock = $connection->prepare($sql_update_stock);
            $stmt_update_stock->bind_param("ii", $reduceAmount, $productId);
            $stmt_update_stock->execute();

            // محاسبه موجودی جدید
            $newStock = $result_check_stock['productCount'] - $reduceAmount;
            ...
            $byUser = $_SESSION['username']; // اطلاعات کاربر از سشن دریافت می‌شود
            $status = "deduction"; // اضافه کردن فیلد status
            ...
            $stmt_insert_report->bind_param("sssssssi", $productName, $productPattern, $productColor, $entryDate, $reduceAmount, $byUser, $status, $newStock);
            $stmt_insert_report->execute();

            $message = "موجودی محصول کاهش یافت و گزارش ثبت شد.";
        } else {
            $error_message = "مقدار وارد شده بیشتر از موجودی محصول است یا محصول یافت نشد.";
        }
    } else {
            ...
            <form method="post">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            <label>تعداد برای کاهش</label>
                            <input type="number" name="reduce_amount" class="form-control" required>
                        </div>
                        <button type="submit" name="reduce_stock" class="btn btn-primary">کاهش موجودی</button>
                    </div>
                </div>
            </form>
        </div>
